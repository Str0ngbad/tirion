...
type LinkChoice = "hasLink" | "missingLink";
...
export default function ColumnFilterLink({ columnId, existing, onApply, onRemove, onCancel }: Props) {
  const existingOp = existing?.operator as FilterOperator | undefined;
  const [choice, setChoice] = useState<LinkChoice>(
    existingOp === "missingLink" ? "missingLink" : "hasLink"
  );
  ...
  return (
    <div className="flex flex-col gap-3 p-3 w-44">
      {(["hasLink", "missingLink"] as LinkChoice[]).map((v) => (
        <label key={v} className="flex items-center gap-2 text-xs cursor-pointer">
          ...
          />
          {v === "hasLink" ? "Has link" : "Missing link"}
        </label>
      ))}
