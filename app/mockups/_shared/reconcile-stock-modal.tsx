...
// Shared Reconcile Stock modal — used by Parts Master, Stock Fulfillment, and any
// other surface that needs inline inventory reconciliation without a context switch.
//
// Prop contract:
//   - partNumber / partName: identity display (read-only)
//   - currentStockCount: shown read-only so the planner sees the delta they're making
//   - onConfirm(newCount, reason): caller owns the state update and audit log entry
//   - onClose: dismiss without action
//
// The modal does NOT touch any data store. The caller records the change.
...
export default function ReconcileStockModal({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (newCountStr.trim() === "" || isNaN(parsed) || parsed < 0 || !Number.isFinite(parsed)) {
