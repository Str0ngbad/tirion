...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const processTypeSubStatusId = Number(id);
    if (!Number.isInteger(processTypeSubStatusId) || processTypeSubStatusId <= 0) {
    ...
    const entries = await prisma.auditLog.findMany({
      where: { entityType: "ProcessTypeSubStatus", entityId: processTypeSubStatusId },
      include: {
