...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const procurementCategoryId = Number(id);
    if (!Number.isInteger(procurementCategoryId) || procurementCategoryId <= 0) {
    ...
    const entries = await prisma.auditLog.findMany({
      where: { entityType: "ProcurementCategory", entityId: procurementCategoryId },
      include: {
