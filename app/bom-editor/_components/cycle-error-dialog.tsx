...
import type { AllActivePartOption } from "@/lib/api/parts";

type Props = {
  open: boolean;
  chain: number[];
  allParts: AllActivePartOption[];
  onClose: () => void;
  buttonLabel?: "OK" | "Cancel";
};

export function CycleErrorDialog({ open, chain, allParts, onClose, buttonLabel = "Cancel" }: Props) {
  const router = useRouter();

  function partLabel(partId: number) {
          ...
          {chain.map((partId, i) => {
            ...
            return (
              <div key={i} className="flex items-start gap-1">
                <span className="whitespace-pre text-muted-foreground">{"  ".repeat(i)}</span>
                {i > 0 && (
