...
interface RemoveChildrenConfirmDialogProps {
  open: boolean;
...
export function RemoveChildrenConfirmDialog({
  open,
  parentPartNumber,
  items,
  onCancel,
  onConfirm,
}: RemoveChildrenConfirmDialogProps) {
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onCancel(); }}>
      <DialogContent className="max-w-sm">
        ...
        <ul className="my-1 space-y-1 rounded border border-border bg-muted/40 p-2 text-xs">
          {items.map((item) => (
            <li key={item.bomId} className="font-mono">
