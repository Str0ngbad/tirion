# project_tracker.md

> Auto-generated. Do not edit by hand.
> Regenerated on every commit by `.claude/hooks/update_tracker.sh`

---

## Phase 0 — Scaffolding
**Status:** Done
- Next.js scaffold with TypeScript, Tailwind, ESLint ✓
- Prisma with adapter-pg, schema applied to Neon ✓
- Pino logging and Sentry error tracking configured ✓
- Vitest sample test passing ✓
- /api/v1/health endpoint deployed to Vercel ✓
- LICENSE and README in place ✓

## Phase 0a — Tooling and Hooks Setup
**Status:** Done
- Self-review hook ✓
- Manifest hook ✓
- Tracker hook ✓
- Deviations hook ✓
- ADRs 001–012 ✓

## Phase 1A — Configuration Foundation
**Status:** Not Started

## Phase 1B — Parts Master
**Status:** Not Started

## Phase 1C — Routing Template Editor
**Status:** Not Started

## Phase 1D — BOM Editor
**Status:** Not Started

## Phase 2 — Spreadsheet Import
**Status:** Not Started

## Phase 3 — Project Creation: Drafts and Compilation
**Status:** Not Started

## Phase 4 — Stock Fulfillment
**Status:** Not Started

## Phase 5 — Batching Lens
**Status:** Not Started

## Phase 6A — Lens Infrastructure
**Status:** Not Started

## Phase 6B — Per-Process Lenses
**Status:** Not Started

## Phase 7 — Operations Lens
**Status:** Not Started

## Phase 8 — Project View + Active Project Management
**Status:** Not Started

## Phase 9 — Blocker Workflows
**Status:** Not Started

## Phase 10 — Polish and Acceptance
**Status:** Not Started

---

## Open DEVIATIONS stubs

_None._

